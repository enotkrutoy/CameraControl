...
export const Camera3DControl: React.FC<Props> = ({ state, onChange }) => {
  ...
  // Use refs for values needed in event listeners to avoid closure staleness
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (isRotatingGizmo) {
        ...
        // Direct manipulation logic for ring rotation
        onChange({ 
  ...
  useEffect(() => {
    if (modelCameraRef.current) {
      ...
      if (gizmoRingRef.current) {
        // Keep the ring centered on the camera group
        gizmoRingRef.current.position.copy(modelCameraRef.current.position);
        // Make the ring visual follow the rotation visually if desired, 
        // but often gizmos stay axis-aligned or follow strictly.
        // For Y rotation, we just need it in the XZ plane.
      }
    }
  }, [state]);
    ...
    <div className="relative border border-gray-700 rounded-2xl overflow-hidden shadow-2xl bg-gray-950 group">
      ...
      <div className="absolute top-4 left-4 flex flex-col gap-2 pointer-events-none">
        ...
        <div className="bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg text-[10px] text-gray-400 font-mono border border-gray-800">
          POS_Z: {state.forward.toFixed(2)} | ROT_Y: {state.rotate.toFixed(1)}°
        </div>
      </div>
      ...
      <div className="absolute top-4 right-4 bg-blue-500/10 border border-blue-500/20 rounded-lg px-2 py-1 flex items-center gap-2 pointer-events-none">
        ...
        <span className="text-[9px] font-bold text-blue-400 uppercase tracking-tighter">Ring Gizmo Enabled</span>
      </div>
      ...
      <div className="absolute bottom-4 right-4 flex items-center gap-4 pointer-events-none">
        <div className="flex gap-1">
          ...
          <div className={`w-1 h-3 rounded-full ${state.tilt !== 0 ? 'bg-indigo-500' : 'bg-gray-700'}`} />
          <div className={`w-1 h-3 rounded-full ${state.rotate !== 0 ? 'bg-emerald-500' : 'bg-gray-700'}`} />
        </div>
        <span className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Spatial Engine v2</span>
      </div>
    </div>
  );
};
